...
## Физическая связь двух объектов (не интеграция)

Для отображения физического соединения двух объектов, например, брандмауэра и коммутатора/роутера используем вид связи "точка-точка". Через него также могут идти интеграционные потоки, но само по себе физическое соединение интеграцией не является.
...
![Пример физической связи двух объектов](@document/dochub.smartants.examples.physical-connection)

## Интеграция через несколько объектов

В случае, если есть необходимость показать связь начального и конечного объектов схемы без детализации на проходящих между ними цепочках интеграций, используем пунктирную линию. Например, связь между пользователем и БД.
...
![Пример интеграции через несколько объектов](@document/dochub.smartants.examples.through-objects)

## Обоюдотупые стрелки (без наконечников)

Для обозначения связей между объектами, когда известно о наличии связи, но нет информации о ее направлении/зависимости.
...
![Пример обоюдотупых стрелок](@document/dochub.smartants.examples.without-tips)
